    ...
    <div className="pointer-events-none fixed top-6 right-6 z-[200] flex flex-col gap-3">
      {toasts.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className={`pointer-events-auto flex max-w-sm items-center gap-3 rounded-2xl border bg-white px-4 py-3 shadow-xl ${wrapClass}`}
          >
            <Icon size={18} className={`shrink-0 ${iconClass}`} />
            <p className="flex-1 text-sm font-medium text-gray-800">{t.message}</p>
            <button
              onClick={() => dismiss(t.id)}
              className="ml-1 rounded-md p-0.5 text-gray-400 transition-colors hover:text-gray-600"
