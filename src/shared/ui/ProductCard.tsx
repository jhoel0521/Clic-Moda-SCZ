...
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <Link href={ROUTES.PRODUCT(product.slug)} className="group block">
      <article className="overflow-hidden rounded-2xl border border-border bg-surface shadow-[0_0_0_0_transparent] transition-all duration-300 hover:-translate-y-1 hover:border-border-brand hover:shadow-brand">
        {/* Imagen */}
        ...
        <div className="p-5 space-y-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-base font-bold text-text-primary">Bs. {product.price}</span>
            {hasDiscount && (
